...
def fetch_genome(acc, email, output_fasta):
    ...
    try:
        ...
        logging.error(f"Error fetching genome: {e}")
        return None

    cds_count = sum(1 for feature in record.features if feature.type == "CDS")
    logging.info(f"The genome {acc} has {cds_count} CDS.")
...
def build_kallisto_index(fasta_file, index_file):
    ...
    command = ["kallisto", "index", "-i", index_file, fasta_file]
    try:
        subprocess.run(command, check=True)
    except subprocess.CalledProcessError as e:
        logging.error(f"Error building Kallisto index: {e}")
...
def run_kallisto_quant(index_file, srr_ids):
    ...
    for id in srr_ids:
        ...
        try:
            ...
            logging.error(f"Error running Kallisto quant for {id}: {e}")
...
def process_abundance(samples, log_file):
    ...
    with open(log_file, "a") as log:
        ...
        for sample_id, condition in samples.items():
            file_path = f"results/{sample_id}/abundance.tsv"
            try:
...
def run_r_analysis(log_file):
    ...
    try:
        ...
        logging.error(f"Error running R script: {e}")
        return
    ...
    try:
        ...
        df_filtered = df[df['qval'] < 0.05] # Filter significant results
        print(df_filtered)
        # Append results without rewriting the header if the file already exists
...
def run_bowtie2_build():
    '''Build Bowtie2 index'''
    command = ["bowtie2-build", "genomic.fna", "bowtie_index"]
    try:
        subprocess.run(command, check=True)
    except subprocess.CalledProcessError as e:
...
def run_bowtie2_mapping(ids, samples, patient_ids, log_file):
    ...
    subprocess.run(["mkdir", "-p", "bowtie2_results"])
    for id in ids:
        ...
        try:
            ...
            logging.error(f"Error running Bowtie2 for {id}: {e}")
...
def concatenate_and_run_spades(patient_ids, log_file):
    ...
    bowtie2_dir = os.path.abspath("bowtie2_results")
    ...
    with open(log_file, "a") as log:
        ...
        for donor, ids in patient_ids.items():
            ...
            # Concatenate all the aligned 1 files
            with open(concatenated_1, 'w') as outfile:
                for fname in aligned_1_files:
                    with open(fname) as infile:
                        outfile.write(infile.read())

            # Concatenate all the aligned 2 files
            ...
            # Run SPAdes
            spades_command = f"spades.py -k 77 -t 1 --only-assembler -1 {concatenated_1} -2 {concatenated_2} -o {donor}_assembly/"
            log.write(f"{spades_command}\n")
            try:
                subprocess.run(spades_command, shell=True, check=True)
            except subprocess.CalledProcessError as e:
                logging.error(f"Error running SPAdes for {donor}: {e}")
...
def parse_samples(file_path):
    ...
    # Create patient ID groups based on condition
    condition_groups = defaultdict(list)
    
    for _, row in df.iterrows():
        sample_id = row["path"].split("/")[-1]  # Extract sample ID from the path
        condition_groups[row["condition"]].append(sample_id)
    
    # Create patient ID lists
...
# python wrapper.py --acc NC_006273.2 --email <YOUR_EMAIL> --srr <SRR_ID_1> <SRR_ID_2> --step all
...
'''I wonder if i can parse their samples.txt file so that it matches anything they do for the spades command'''
